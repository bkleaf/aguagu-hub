...
class CardTransactionController(
    ...
    @Operation(summary = "파싱 실패 거래 조회")
    @GetMapping("/failed")
    fun getFailedTransactions(): ResponseEntity<List<CardTransactionResponse>> {
        val transactions = cardTransactionService.getFailedTransactions()
        return ResponseEntity.ok(transactions)
    }

    @Operation(summary = "파싱 성공 거래 조회")
    @GetMapping("/successful")
    fun getSuccessfulTransactions(): ResponseEntity<List<CardTransactionResponse>> {
        val transactions = cardTransactionService.getSuccessfulTransactions()
        return ResponseEntity.ok(transactions)
    }
    ...
    fun getSupportedCardCompanies(): ResponseEntity<List<Map<String, String>>> {
        val companies = cardTransactionService.getSupportedCardCompanies()
            .map { mapOf("code" to it.name, "name" to it.displayName) }
