 ...
 * KB국민카드 월별 청구서 문자 파서
 *
 * KB국민카드의 월별 결제 청구 문자를 파싱합니다.
 *
 * 형식 예시:
 * ```
 * 2/25 카드대금 150,000원 (2/10기준)
 * ```
 */
@Component
class KbBillParser : BillMessageParser {

    companion object {
        /**
         * KB카드 청구서 정규식
         * 그룹: (결제월)/(결제일) 카드대금 (금액)원 ((기준월)/(기준일)기준)
         */
        private val BILL_PATTERN = Regex(
            """(\d{1,2})/(\d{1,2})\s*카드대금\s*([0-9,]+)원\s*\((\d{1,2})/(\d{1,2})기준\)"""
        )
    }
    ...
    override fun parse(message: String): BillParseResult {
        return try {
            ...
            BillParseResult.failure("KB카드 청구서 파싱 중 오류: ${e.message}")
        }
    }
